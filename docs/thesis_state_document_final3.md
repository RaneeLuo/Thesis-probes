...
**TRACE narrative probe (2026-08-09) — MEASUREMENT COMPLETE AND REPLICATED; HARDENING OPEN.** Runner built from source reads (description path verified independent, G7 diff 4.5e-07; authors' eval uses a RANDOM 30% mask — replicated over seeded draws 13/14/15, unperturbed P@1 0.428–0.441, spread 0.0125). All gates green after one caught bug (error #10). Results (3,178 items × 3 seeds; no VOID; random condition 0.990–1.000): swap accuracy / gap — N1 0.875–0.892 / +0.11; N2 0.935–0.950 / +0.055; **N3 0.720–0.725 / +0.27 (largest gap; margins 0.005; internal duration gradient week 0.648 → six-months 0.841 must be reported with any N3 claim)**; N5 0.917–0.935 / +0.074. All Holm-significant, all seeds. TRACE is not CLaSP: partial degradation everywhere, no collapse — hard negatives help but do not eliminate component-swap sensitivity. **N5 reframed:** the designed negative control detects location at 0.92 — and at **0.900 on the 40 place-name-only swaps** (frame confound ruled out; length ruled out) — so location IS signal-inferable to TRACE; climate-inference vs station-memorization left open; not reportable as a negative control; supervisor item. Length confound closed (N1/N3 swap sets zero-word-diff by construction; flagged cells deflationary). Prediction ledger: 6 confirmed, 2 missed, 1 partial — misses recorded (handoff §4.5). **Hardening: the N3 screen FAILED and the full census is engaged** (sample judged same-day: primary 86/100, failing at batch-1; two-pass union → 17 known defects, rate 0.170 CI [0.109, 0.256]; five defect mechanisms codified; rule-certification refuted by counterexample N3|1577; Ranyi judging all 289 remaining flagged-population items — census sheet generated; verdict, excision with twins, and N3 recomputation = next session's first task; N3 numbers provisional until then; known mechanisms bias toward UNDERSTATING the gap, an argument not a certification). Canonical records: `results/experiments/trace_narrative_{per_item.jsonl,summary.json,statistics.json}`, `results/analysis/trace_narrative_slices.json`, `results/analysis/probe1_item_balance_trace_narrative.json`.

**Not started:** Probe 1 on TRUCE substrate; ChatTS; Probes 2 and 3.

**Immediate next steps, in order:**
1. **N3 census verdict** (the 100-item screen already FAILED its pre-registered criterion — 86/100 primary, 17 union defects; census sheet of the 289 remaining items is with Ranyi for offline judging). Next session: score the filled census, excise defective items with their matched random twins, recompute all N3 statistics and the duration gradient on the certified survivors, and issue the certification verdict — only then do N3 numbers become quotable. Then: supervisor conversation (N5 reframing, threat revision, two-walled C4, N3 gradient + census outcome, sample-to-screen/census-to-certify arc now demonstrated twice).
...
| **TRACE** (Chen et al., arXiv 2506.09114) | multivariate retriever, CIT + channel-biased attention, K=32, λ_ch=1.0 | NOAA test-split descriptions via the authors' generate_dsp template (largely LLM channel prose — the "human narratives" framing was corrected 2026-08-08, see §2; narrative grammar N1–N5) | binary forced choice, acc gap swap-vs-random | substrate resolved (b); 3,178 certified items (N1/N2/N3/N5; N4 unbuildable-clean, dropped); measurement complete 2026-08-09, replicated over 3 mask seeds; N5 reframed (not a negative control); hardening open — N3 human sample pending |

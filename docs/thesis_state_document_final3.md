...
**Not started:** Probe 1 on TRUCE substrate; ChatTS; Probe-2 EXECUTION (design complete 2026-08-13, all groupings certified, nothing run); Probe 3.
...
4. **Probe-2 execution status:** CLaSP arm DONE AND SCORED (2026-08-13; results record in §4 and handoff §4.7): P2-1 CONFIRMED (SUSHI dep sf-all Δ 0.232/0.239/0.280, rel 76–78%, Wilcoxon p ≤ 4e-16), P2-2 CONFIRMED (TRUCE DiD +0.0022/+0.0762/+0.0729; 95% CIs exclude 0 in seeds 43/44), P2-3 CONFIRMED, P2-5 MISSED (TOST ±0.05 pinned pre-analysis: PASS in seeds 43/44, seed 42 fails via ONE query — 'The majority is flat.' → rank 1→259; registered interpretation instantiated). **Floor arm DONE AND SCORED 2026-08-14: P2-4 CONFIRMED** (runner v2 with D2-F; 24/24 TOSTs at ±0.05 abs over both inference cells × 4 perts × 3 arms; max |Δ| 0.0105; $0.75 spent). Named finding recorded: floor TRUCE DiD +0.010/+0.084/+0.072 with CIs excluding 0 in arms 43/44 — CLaSP-P2-2's surface signature on a no-capability model, invariant-driven (thin-cell improvement distributed over 4–6 stock-signal queries; OW_5#2 holds the cell's only rank-1 query) where CLaSP's is dependent-driven; the decomposition/per-group-baseline rules are the discriminator. Prediction ledger: tie count 24 HIT; cluster structure, G8-empty, and leave-out-OW_5 MISSED with mechanisms (log 2026-08-14). NEXT (new chat): TRACE runner (P2-9 + profile).

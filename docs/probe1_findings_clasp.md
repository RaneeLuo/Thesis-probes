...
**Caption length — measured, and shown not to drive the result.** Mean absolute word-count difference between correct and swapped captions: C1 0.32, C3 0.17, C4 0.17 — negligible; C5 2.46 and C2 5.11 — substantial, because cubic and periodic descriptions are genuinely longer than linear ones. A length cue would make the distractor *easier* to reject and therefore inflate apparent sensitivity, so it works against finding a shortcut. The two confounded components are also the two with the smallest gaps, and the ordering C1 < C3 < C4 holds entirely within the length-clean components.

Two further checks close this threat rather than merely bounding it. First, an oracle that ignores content entirely and always selects the longer caption scores 0.495–0.517 across every swap condition (`probe1_item_balance.json`) — within 0.017 of chance, so length is not exploitable in the condition from which every finding here is drawn. Second, correlating the model's margin against the caption-length difference gives **+0.023 overall in the swap condition** (per component: +0.109, −0.053, +0.048, −0.078, +0.044), i.e. essentially no relationship. The contrast with the floor baseline is instructive: on the identical items, `text-embedding-3-large` yields +0.174, and its accuracy falls below chance precisely where correct captions are shorter. **The reimplementation is therefore demonstrably not using the length heuristic, and C4's correlation of −0.078 shows its fluctuation failure is a genuine encoding gap rather than a surface artefact.**

...
## 4.6 Probe 2 (order-invariance shuffle) — design questions for the NEXT stage (opened 2026-08-09, nothing resolved yet)
...
5. **Gates to build in:** shuffle-actually-applied check (serialisation diff per item), grouping-coverage counters, direction-of-degradation sanity on a known-order-dependent stratum, and the standing registered-expectation-per-command rule.

Prediction (registered now, mechanism-level, to sharpen next session): CLaSP degrades on order-dependent captions under sf-all and shows the DiD signature; the interesting open question is whether the order-INVARIANT group stays flat (TOST) — that flatness is what makes the differential publishable-clean rather than trivial.

...
1. **Re-read Tan et al. §4.4 from source first** (alphaXiv, one batched call): exact shuffle granularities and the masking perturbation's parameters. Only the counts (3+1) are verified in §6.1; the mechanics are NOT yet pinned from source. Do not design from memory.
2. **Metric decision per model:** strict-retrieval rank shift (uses the existing baselines: CLaSP frozen table, TRACE demo repro, floor model's new baseline) vs forced-choice items in the Probe-1 style. This decision shapes everything downstream; argue it, pre-register it, record it in PROJECT_CONTEXT if it becomes binding.
3. **Caption grouping per substrate:** SUSHI — likely rule-based from labels (which of the 7 fluctuation / 20 shape families count as order-invariant? candidate: fluctuation-only clauses; shape clauses are order-dependent almost by definition — check the imbalance this creates). TRUCE — needs the 3-way classifier + human sample (parse-coverage reported, selection bias stated). TRACE narrative — which sentence types are order-invariant (condition labels? statistics?) and does the 186-point substrate leave enough order-dependent signal after the §4.1 downsample findings?
4. **Shuffle mechanics per substrate:** point-level vs block-level on length-2048 SUSHI; what sf-half/ex-half mean on 12-point TRUCE series; TRACE multichannel — shuffle within channels jointly or independently (jointly, presumably — argue it).
